...
> `npm test` 495 / 495 · `npm run build` Cloudflare server bundle complete.
...
| Gate | Result |
| --- | --- |
| `npm test` | **495 / 495 passing**, with zero notification writes failing open unnoticed (grepped for after the A19 review found four fixtures swallowing them) |
| `npm run check` | 370 files · 0 errors · 0 warnings · 0 hints |
...
**Dashboard figures** — "Omset" is the value of orders still in play: cancelled orders and released payments are excluded, and a separate collected-revenue figure counts paid orders only, so a store with no paid orders no longer reports a seven-figure "pendapatan". Payment success is measured over online orders only (COD is paid on delivery and cannot prepay) and RTS over shipments that reached an outcome; every card states its base. The default reporting period is this month. The trend chart's axis scales per tick so labels stay distinct, the all-time view fills empty days rather than skipping them, and a payment provider that has accepted requests but awaits its first manual confirmation reads healthy, not "no data".
