...
Missing published home content fails closed. `getTenantHomeContent` returns
`setup-required`, and `/` plus `GET /api/v1/storefront` expose only the installed
store identity and an explicit setup state until the operator publishes home
