...
**Dashboard figures** — "Omset" is the value of orders still in play: cancelled orders and released payments are excluded, and a separate collected-revenue figure counts paid orders only, so a store with no paid orders no longer reports a seven-figure "pendapatan". Payment success is measured over online orders only (COD is paid on delivery and cannot prepay) and RTS over shipments that reached an outcome; every card states its base. The default reporting period is this month.
