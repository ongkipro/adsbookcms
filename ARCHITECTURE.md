...
| G5 | Home content falls back to built-in copy | A fresh install shows a generic storefront instead of an honest setup state; `getTenantHomeContent` never returns `null`, so the setup branch is unreachable (A-12b) | `src/lib/tenant-content.ts` |
